...
# MediFind - Play Store Release Build Script
# This script builds a signed AAB file for Google Play Store

set -e  # Exit on error

echo "🚀 MediFind - Building Release AAB for Play Store"
echo "=================================================="
...
KEYSTORE_PATH="$PROJECT_ROOT/android/app/medifind-release-key.keystore"
if [ ! -f "$KEYSTORE_PATH" ]; then
    ...
    keytool -genkeypair -v -storetype PKCS12 \
        -keystore medifind-release-key.keystore \
        -alias medifind-key-alias \
        -keyalg RSA \
        -keysize 2048 \
        -validity 10000
...
if ! grep -q "MYAPP_RELEASE_STORE_FILE" "$GRADLE_PROPERTIES" 2>/dev/null; then
    ...
    echo ""
    echo "MYAPP_RELEASE_STORE_FILE=medifind-release-key.keystore"
    echo "MYAPP_RELEASE_KEY_ALIAS=medifind-key-alias"
